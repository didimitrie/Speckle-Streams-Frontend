  ...
  <span>
      ...
      <div v-el:wrap :class="['vue-slider-wrap', flowDirection, className, { 'vue-slider-disabled': (isDisabled && this.eventType !== 'none') }]" v-show="show" :style="[( styles || {} ), wrapStyles]" @click="wrapClick">
        <div v-el:elem class="vue-slider" :style="elemStyles">
          <template v-if="isRange">
            <div v-el:dot0 :data-rangeValue="value[0]" :class="[ tooltipStatus, `vue-slider-tooltip-${tooltipDirection}`, 'vue-slider-dot dim-hack']" :style="dotStyles" @touchstart="moveStart(0)"></div>
            <div v-el:dot1 :data-rangeValue="value[1]" :class="[ tooltipStatus, `vue-slider-tooltip-${tooltipDirection}`, 'vue-slider-dot dim-hack']" :style="dotStyles" @touchstart="moveStart(1)"></div>
          </template>
          <template v-else>
            <div v-el:dot :data-rangeValue="value" :class="[ tooltipStatus, `vue-slider-tooltip-${tooltipDirection}`, 'vue-slider-dot dim-hack']" :style="dotStyles" @touchstart="moveStart"></div>
          </template>
    ...
    <template v-else>
      <div v-el:wrap :class="['vue-slider-wrap', flowDirection, className, { 'vue-slider-disabled': (isDisabled && this.eventType !== 'none') }]" v-show="show" :style="[( styles || {} ), wrapStyles]" @click="wrapClick">
        <div v-el:elem class="vue-slider" :style="elemStyles">
          <template v-if="isRange">
            <div v-el:dot0 :data-rangeValue="value[0]" :class="[ tooltipStatus, `vue-slider-tooltip-${tooltipDirection}`, 'vue-slider-dot dim-hack']" :style="dotStyles" @mousedown="moveStart(0)"></div>
            <div v-el:dot1 :data-rangeValue="value[1]" :class="[ tooltipStatus, `vue-slider-tooltip-${tooltipDirection}`, 'vue-slider-dot dim-hack']" :style="dotStyles" @mousedown="moveStart(1)"></div>
          </template>
          <template v-else>
            <div v-el:dot :data-rangeValue="value" :class="[ tooltipStatus, `vue-slider-tooltip-${tooltipDirection}`, 'vue-slider-dot dim-hack']" :style="dotStyles" @mousedown="moveStart"></div>
          </template>
          <template v-if="piecewise">
            <ul v-if="direction === 'vertical'" class="vue-slider-piecewise">
              <li v-for="i in (total - 1)" :style="[ piecewiseStyle, { bottom: `${ gap * (i + 1) - width / 2 }px`, left: '0px' }]"></li>
  ...
  computed: {
    ...
    total: function() {
      ...
      if ((this.maximum - this.minimum) % this.interval !== 0) {
        // console.error('[Vue warn]: Prop[interval] is strange, Please make sure that the interval can be divisible')
      }
...
.vue-slider-dot {
    ...
    box-shadow: 1.5px 1.5px 8px -1px rgba(0, 0, 0, 0.32);
    transition: all 0.3s;
    cursor: pointer;
    z-index: 3;
}
...
.vue-slider-dot:hover{
  /*box-shadow: 2.5px 1.5px 8px -1px rgba(0, 0, 0, 0.72);*/
}
