...
// import ScatterMatrix from './scatter-matrix'
export default {
  data () {
    return {
      Package: null,
  ...
  ready() {
    var CSVs = '';
    this.$http
    .get( window.SPKCONFIG.apiendpoint + '/api/frontend/package', { params: { packid: this.$route.params.packageid } } )
    .then( (response) => {
      let mypack = response.data.package
      let header = ''
      for(let item of mypack.controllers) {
        header += item.nickname ? item.nickname : item.name
        header += ','
      }
      for(let item of mypack.performances) {
        header += item.name + ','
      }
      header = header.slice(0,-1)
      CSVs += header + '\n'
      for(let item of response.data.package.ParamPerformanceKeys) {
        let row = '', k = 0
      ...
      // console.log( d3.csvParse(CSVs) )
      var sm = new ScatterMatrix('null', CSVs, 'matrix')
      sm.render()
      // ScatterMatrix.render()
      console.log(sm)
    })
    // console.log(CS)
  }
...
var ScatterMatrix = function(url, data, dom_id, el) {
  this.__url = url;
...
.scatter-matrix-control { position: absolute; float: left; margin-right: 20px; width: 250px; display: inline-block;  }
.scatter-matrix-variable-control ul { list-style: none; padding-left: 10px; }
.scatter-matrix-svg { position: absolute; left: 250px; float: left; margin-top: 15px; width: 1000px; }
